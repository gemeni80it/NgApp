...
export class PaginationComponent implements OnInit {
  ...
  getPages() : number[] {
    const totalPages = Math.ceil(this.count / this.perPage);  // Store Total number of pages        ex: 10 pages
    const thisPage = this.page || 1;                          // Actual page                        ex: show page 1
    const pagesToShow = this.pagesToShow || 9;                // Between prev && next => max 9 link pages at the bottom
    const pages: number[] = [];                               
    pages.push(thisPage);
